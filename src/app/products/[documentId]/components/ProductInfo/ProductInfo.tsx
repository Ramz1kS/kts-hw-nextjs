...
function ProductInfo({ product }: ProductInfoProps) {
    ...
    return (
      <div className={classes['product-info']}>
        ...
        >
            <Image className={classes['product-info__image']} 
            fill
            src={product.images.length == 0 ? '' : product.images[currentImageIndex].formats.large.url} 
            loading="eager"
            alt={product.title} />
            {hasMultipleImages && (
              <>
                ...
                <button className={classes['product-info__nav-btn-prev']} onClick={prevImage}>
                  <ArrowLeftIcon width={32} height={32} color="primary" />
                </button>
                <button className={classes['product-info__nav-btn-next']} onClick={nextImage}>
                  <ArrowLeftIcon width={32} height={32} color="primary" />
                </button>
              </>
            )}
        ...
        >
          ...
          <div className={classes['product-info__actions']}>
            ...
            <div className={classes['product-info__buttons']}>
              ...
              oneLined={true}
              >{product.isInStock ? 'Buy now' : 'Not in stock'}</Button>
