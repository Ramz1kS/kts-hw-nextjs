...
import { CategoryData, CategoryResponse, ListResponse, ProductData, ProductListQuery, ProductsListStoreInitData, SortByType, SortHowType } from "@/shared/types";
...
export default class ProductListStore {
    ...
    constructor(initData?: ProductsListStoreInitData, initQuery: ProductListQuery | undefined = undefined) {
        if (initData) {
        ...
        if (initQuery) {
            ...
            const selectedFiltersIds = initQuery.categories !== undefined ? 
            initQuery.categories?.split(',').map(i => parseInt(i, 10)) : [] 

            this.selectedFiltersCategory = this.filtersCategory.filter(c => selectedFiltersIds.includes(c.id))
  ...
  getURLSearchParams(): URLSearchParams {
    ...
    if (this.selectedFiltersCategory.length !== 0) {
        params.set('categories', this.selectedFiltersCategory.map(c => c.id).join(','))
    }
  ...
  static fromJson(initData: ProductsListStoreInitData, initQuery: ProductListQuery) {
